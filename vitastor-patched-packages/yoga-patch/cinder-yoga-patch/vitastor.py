...
# 392-537: implement trash delete for snapshot and volume
...
class VitastorDriver(driver.CloneableImageVD,
    ...
    def delete_snapshot(self, snapshot):
        ...
        """Delay the snapshot deletion."""
        delay = 0
        if self.configuration.enable_deferred_deletion:
            delay = self.configuration.deferred_deletion_delay
        current_timestamp = int(time.time())
        delete_timestamp = current_timestamp + delay
        
        args = [
            'vitastor-cli', 'modify', vol_name+'@'+snap_name,
            '--rename', "TRASH_"+str(delete_timestamp)+"_"+vol_name+'@'+snap_name,
            *(self._vitastor_args())
        ]
        try:
            self._execute(*args)
        except processutils.ProcessExecutionError as exc:
            LOG.error("Failed to trash snapshot "+vol_name+'@'+snap_name+": "+str(exc))
            raise exception.VolumeBackendAPIException(data = exc.stderr)
    ...
    def delete_volume(self, volume):
        ...
        args = [
            'vitastor-cli', 'modify', vol_name,
            '--rename', "TRASH_"+str(delete_timestamp)+"_"+vol_name,
            *(self._vitastor_args())
        ]
        try:
            self._execute(*args)
        except processutils.ProcessExecutionError as exc:
            LOG.error("Failed to trash volume "+vol_name+": "+str(exc))
            raise exception.VolumeBackendAPIException(data = exc.stderr)
